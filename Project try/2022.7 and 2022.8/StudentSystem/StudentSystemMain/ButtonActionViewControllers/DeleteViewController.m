    ...
    if (studentName.length && studentClass.length) {
        if ([self.studentName containsObject:studentName]) {
            ...
            if ([self.studentClass containsObject:studentClass]) {
                UIAlertController* alert = [UIAlertController alertControllerWithTitle:@"提示" message:@"是否删除此人" preferredStyle:UIAlertControllerStyleAlert];
                ...
                [alert addAction:[UIAlertAction actionWithTitle:@"确认" style:UIAlertActionStyleDestructive handler:^(UIAlertAction* action) {
                    for (int i = 0; i < [self.studentData[ARRAY_STUDENTNUMBER] count]; i++) {
                        if ([self.studentData[ARRAY_STUDENTNAME][i] isEqual:studentName] && [self.studentData[ARRAY_STUDENTCLASS][i] isEqual:studentClass]) {
                            ...
                            // 删除数组中的数据
                            [self.studentData[ARRAY_STUDENTNAME] removeObjectAtIndex:i];
                            [self.studentData[ARRAY_STUDENTCLASS] removeObjectAtIndex:i];
                            [self.studentData[ARRAY_STUDENTNUMBER] removeObjectAtIndex:i];
                        }
                    }
        ...
        UIAlertController* alert = [UIAlertController alertControllerWithTitle:@"提示" message:@"请输入要删除的学生信息" preferredStyle:UIAlertControllerStyleAlert];
